...
## Appendix A: 补充设计（Kimi 优化建议）

### A.1 Agent trait 详细定义

```rust
// cis-common/cis-traits/src/agent.rs
use async_trait::async_trait;
use crate::types::{AgentConfig, AgentType, AgentStatus};
use crate::traits::{Lifecycle, Named, MemoryLoader};

/// Core Agent trait for CIS multi-agent system
#[async_trait]
pub trait Agent: Send + Sync + Lifecycle + Named {
    ...
    /// Get memory loader for context building
    ...
    fn can_handle(&self, task_type: &TaskType) -> bool {
        // Default implementation based on agent_type
        matches!((self.agent_type(), task_type),
            (AgentType::Coder, TaskType::Code) |
            (AgentType::Doc, TaskType::Documentation) |
            (AgentType::Debugger, TaskType::Debugging) |
            (AgentType::Receptionist, TaskType::Simple)
        )
    }
}
...
#[async_trait]
pub trait AgentPool: Send + Sync + Lifecycle + Named {
    /// Acquire an agent of specified type
...
pub trait AgentFactory: Send + Sync {
    fn create(&self, config: AgentConfig) -> anyhow::Result<Box<dyn Agent>>;
}
...
/// Usage example:
/// ```rust
/// let task = TaskBuilder::new("deploy-001", "Deploy to production")
///     .with_level(TaskLevel::Arbitrated { 
///         stakeholders: vec!["devops".to_string(), "manager".to_string()] 
///     })
///     .with_agent("remote_deployer")
///     .with_dependencies(vec!["test-001".to_string(), "build-001".to_string()])
///     .with_timeout(600)
///     .build()?;
/// ```
...
impl MemoryEntryBuilder {
    ...
    pub fn for_agent(mut self, agent: impl Into<String>) -> Self {
        self.agent_namespace = Some(agent.into());
        self
    }
    ...
    pub fn build(self) -> anyhow::Result<MemoryEntry> {
        if self.key.is_empty() {
            return Err(anyhow::anyhow!("Memory key cannot be empty"));
        }
...
### A.3 CIS ↔ ZeroClaw 类型映射表

| CIS Type | ZeroClaw Type | 映射说明 | 实现 |
|----------|---------------|----------|------|
| `MemoryDomain::Private` | `MemoryCategory::Core` | 私域 → Core | `impl From<MemoryDomain> for MemoryCategory` |
| `MemoryDomain::Public` | `MemoryCategory::Context` | 公域 → Context | `impl From<MemoryDomain> for MemoryCategory` |
...
| `TaskLevel::Mechanical` | `ExecutionMode::Auto` | 自动执行 | `impl From<TaskLevel> for ExecutionMode` |
| `TaskLevel::Recommended` | `ExecutionMode::Suggest` | 建议模式 | `impl From<TaskLevel> for ExecutionMode` |
| `TaskLevel::Confirmed` | `ExecutionMode::Confirm` | 确认模式 | `impl From<TaskLevel> for ExecutionMode` |
| `TaskLevel::Arbitrated` | `ExecutionMode::Arbitrate` | 仲裁模式 | `impl From<TaskLevel> for ExecutionMode` |
...
/// TaskLevel → ExecutionMode
impl From<cis_types::TaskLevel> for zeroclaw::scheduler::ExecutionMode {
    fn from(level: cis_types::TaskLevel) -> Self {
        match level {
            cis_types::TaskLevel::Mechanical { .. } => Self::Auto,
            cis_types::TaskLevel::Recommended { .. } => Self::Suggest,
            cis_types::TaskLevel::Confirmed => Self::Confirm,
            cis_types::TaskLevel::Arbitrated { .. } => Self::Arbitrate,
        }
    }
}

/// Custom type handling
impl From<zeroclaw::memory::MemoryCategory> for cis_types::MemoryCategory {
    fn from(category: zeroclaw::memory::MemoryCategory) -> Self {
        match category {
            zeroclaw::memory::MemoryCategory::Core => Self::Context,
            zeroclaw::memory::MemoryCategory::Daily => Self::Context,
            zeroclaw::memory::MemoryCategory::Conversation => Self::Context,
            zeroclaw::memory::MemoryCategory::Custom(name) => {
                // Heuristic mapping based on name
                if name.contains("skill") || name.contains("tool") {
                    Self::Skill
...
[features]
default = ["std", "async", "memory", "scheduler"]
std = ["cis-types/std"]
...
zeroclaw-compat = ["memory", "scheduler"]  # ZeroClaw compatibility layer

# cis-common/cis-memory/Cargo.toml
[features]
default = ["std", "async", "storage", "vector"]
std = ["cis-types/std", "cis-traits/std"]
async = ["cis-traits/async"]
storage = ["dep:cis-storage"]

# Vector search capabilities
vector = ["sqlite-vec", "embedding"]
embedding = ["dep:fastembed", "vector"]
vector-search = ["sqlite-vec"]  # Only search, no embedding generation

# Sync capabilities
sync = ["dep:cis-p2p", "encryption"]
encryption = ["dep:ring"]

# P2P capabilities (for remote agents)
p2p = ["sync", "encryption", "dep:cis-p2p"]

# cis-common/cis-scheduler/Cargo.toml
[features]
default = ["std", "async", "dag", "federation"]
std = ["cis-types/std", "cis-traits/std"]
async = ["cis-traits/async"]

# DAG capabilities
dag = []
federation = ["p2p", "dep:cis-p2p"]
p2p = []
...
decision-confirmed = []
decision-arbitrated = ["federation"]
all-decisions = ["decision-mechanical", "decision-recommended", "decision-confirmed", "decision-arbitrated"]
...
# Core features
encryption = ["cis-memory/encryption"]
vector = ["cis-memory/vector"]
p2p = ["cis-memory/p2p", "cis-scheduler/p2p", "dep:cis-p2p"]
wasm = []
parking_lot = ["dep:parking_lot"]

# Multi-agent support
multi-agent = ["agent-pool", "receptionist", "worker-agents"]
agent-pool = ["cis-traits/agent"]
receptionist = ["agent-pool"]
worker-agents = ["agent-pool", "coder", "doc", "debugger"]
coder = []
doc = []
debugger = []

# Optional integrations
zeroclaw = ["dep:zeroclaw", "cis-memory/zeroclaw-compat", "cis-scheduler/zeroclaw-compat"]
```
...
### 相对于 GLM v3.2 Final 的变更

1. **Appendix A.1**: 补充了 Agent trait 和 AgentPool 的详细定义
2. **Appendix A.2**: 补充了 TaskBuilder 和 MemoryEntryBuilder 的实现
3. **Appendix A.3**: 补充了 CIS ↔ ZeroClaw 类型映射表和实现代码
4. **Appendix A.4**: 补充了详细的 Feature Flag 设计

### 审查确认

- ✅ 三层架构设计完整
- ✅ 多 Agent 架构（Phase 7）完整
- ✅ ZeroClaw 兼容性策略明确
- ✅ 四级决策 + DAG 编排充分体现 CIS 特色
- ✅ P2P 跨设备调用设计完整
- ✅ 所有补充内容已添加至附录
...
**整合完成时间**: 2026-02-20
**整合者**: Kimi
**基础版本**: GLM CIS_V1.2.0_FINAL_PLAN_glm.md v3.2 Final
**状态**: ✅ **已整合全部需求设计，可进入实施阶段**
